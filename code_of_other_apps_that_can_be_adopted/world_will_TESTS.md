...
### Unit Tests
```python
# tests/unit/test_world_will.py
import pytest
from systems.world_will import WorldWill

@pytest.fixture
def instance():
    return WorldWill(data_path="tests/data")
...
```bash
# All tests
python -m pytest tests/unit/test_world_will.py -v

# With coverage
python -m pytest tests/unit/test_world_will.py --cov=systems.world_will

# Specific test
python -m pytest tests/unit/test_world_will.py::test_basic_operation -v
```
