...
### Unit Tests
```python
# tests/unit/test_raven_rag.py
import pytest
from yggdrasil.ravens.raven_rag import RavenRag

@pytest.fixture
def instance():
    return RavenRag(data_path="tests/data")
...
```bash
# All tests
python -m pytest tests/unit/test_raven_rag.py -v

# With coverage
python -m pytest tests/unit/test_raven_rag.py --cov=yggdrasil.ravens.raven_rag

# Specific test
python -m pytest tests/unit/test_raven_rag.py::test_basic_operation -v
```
