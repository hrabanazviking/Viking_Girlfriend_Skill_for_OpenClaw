...
### Unit Tests
```python
# tests/unit/test_muspelheim.py
import pytest
from yggdrasil.worlds.muspelheim import Muspelheim

@pytest.fixture
def instance():
    return Muspelheim(data_path="tests/data")
...
```bash
# All tests
python -m pytest tests/unit/test_muspelheim.py -v

# With coverage
python -m pytest tests/unit/test_muspelheim.py --cov=yggdrasil.worlds.muspelheim

# Specific test
python -m pytest tests/unit/test_muspelheim.py::test_basic_operation -v
```
